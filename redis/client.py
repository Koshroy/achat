...
def list_or_args(keys, args):
    ...
    #     keys.extend(args)
    # return keys
    print 'loa debug: ' + str(keys) + ' ' + str(args)
    if (not isinstance(keys, basestring)) and isinstance(keys, collections.Sequence):
        if not isinstance(keys, collections.MutableSequence):
            keys = list(keys)
    else:
        keys = [keys]
    if args:
...
class StrictRedis(object):
    ...
    def sadd(self, name, *values):
        ...
        print 'debug sadd: ' + str(name) + ' ' + str(values)
